...
export default function StudentSidebar({ year, currentPage, totalPages }: StudentSidebarProps) {
 ...
 {s.major && <div className="text-xs text-text-muted truncate">{s.major}</div>}
 </div>
 </div>
 ))}
 </div>
 ...
 <p className="text-xs text-text-muted">No students assigned to pages yet.</p>
 </div>
 ) : (
 <div className="py-1">
 {allPages.map((page) => {
 ...
 className="md:hidden fixed bottom-20 right-4 z-30 bg-primary text-text rounded-full p-3 shadow-md hover:bg-primary-hover transition-colors"
 title="Show students"
 >
 ...
 {/* Desktop sidebar */}
 <div className="hidden md:flex fixed right-0 top-0 h-full z-30 w-72 flex-col bg-surface border-l border-border">
 <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
 <div className="flex items-center gap-2">
 <Users className="w-4 h-4 text-primary" />
 <span className="text-sm font-semibold text-text">Students</span>
