  ...
  if (!collapsed && mobileOpen) {
    return (
      <div className="fixed inset-0 z-40 md:hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 max-h-[70vh] bg-black/90 backdrop-blur-xl border-t border-white/10 rounded-t-2xl flex flex-col">
                ...
                <div className="space-y-2">
                  {currentPageStudents.map((s) => (
                    <div key={s.id} className="flex items-center gap-2.5">
                      {s.avatar_image_url ? (
                        <img src={s.avatar_image_url} alt={s.full_name} className="w-7 h-7 rounded-full object-cover border border-white/20 shrink-0" />
                      ) : (
                      ...
                      <div className="min-w-0">
                        ...
                        {s.major && <div className="text-xs text-white/40 truncate">{s.major}</div>}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {allPages.map((page) => {
                  ...
                  return (
                    <div key={page} className={isCurrent ? "bg-white/5" : ""}>
                      <button onClick={() => togglePage(page)} className="w-full flex items-center justify-between px-4 py-2 hover:bg-white/5 transition-colors">
                        <div className="flex items-center gap-2">
                          <ChevronRight className={`w-3 h-3 text-white/40 transition-transform ${isExpanded ? "rotate-90" : ""}`} />
                          <span className={`text-xs font-medium ${isCurrent ? "text-[#e94560]" : "text-white/60"}`}>Page {page}</span>
                        </div>
                        ...
                        <div className="pb-2">
                          {students.map((s) => (
                            <div key={s.id} className="flex items-center gap-2.5 px-4 py-1.5 pl-10">
                              {s.avatar_image_url ? (
                                <img src={s.avatar_image_url} alt={s.full_name} className="w-5 h-5 rounded-full object-cover border border-white/20 shrink-0" />
                              ) : (
                              ...
                              <div className="min-w-0">
                                ...
                                {s.major && <div className="text-[10px] text-white/30 truncate">{s.major}</div>}
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          <div className="px-4 py-2 border-t border-white/10 shrink-0">
            <div className="text-[10px] text-white/30 text-center">{allPages.length} pages with students</div>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <>
        ...
        <button
          ...
          className="md:hidden fixed bottom-20 right-4 z-30 bg-[#e94560] text-white rounded-full p-3 shadow-lg shadow-[#e94560]/30 hover:bg-[#d63d56] transition-colors"
          title="Show students"
        >
        ...
        {/* Desktop sidebar */}
        <div className="hidden md:flex fixed right-0 top-0 h-full z-30 w-72 flex-col bg-black/40 backdrop-blur-xl border-l border-white/10">
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 shrink-0">
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4 text-[#e94560]" />
              <span className="text-sm font-semibold text-white">Students</span>
