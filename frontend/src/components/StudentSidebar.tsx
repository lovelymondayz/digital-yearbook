...
export default function StudentSidebar({ year, currentPage, totalPages }: StudentSidebarProps) {
  ...
  if (!collapsed && mobileOpen) {
    return (
      <div className="fixed inset-0 z-40 md:hidden">
        <div className="absolute inset-0 bg-surface-alt" onClick={() => setMobileOpen(false)} />
        <div className="absolute bottom-0 left-0 right-0 max-h-[70vh] bg-surface  border-t border-border rounded-t-2xl flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
                  ...
                  {currentPageStudents.map((s) => (
                    <div key={s.id} className="flex items-center gap-2.5">
                      {s.avatar_image_url ? (
                        ...
                        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[primary] to-[primary] flex items-center justify-center text-xs font-bold text-text shrink-0">
                          {s.full_name.charAt(0).toUpperCase()}
                        </div>
                      )}
                      <div className="min-w-0">
                        <div className="text-sm text-text truncate">{s.full_name}</div>
                        {s.major && <div className="text-xs text-text/40 truncate">{s.major}</div>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {loading ? (
              ...
              <div className="px-4 py-8 text-center">
                <Users className="w-8 h-8 text-text/20 mx-auto mb-2" />
                <p className="text-xs text-text/40">No students assigned to pages yet.</p>
              </div>
            ) : (
              <div className="py-1">
                {allPages.map((page) => {
                  ...
                  return (
                    <div key={page} className={isCurrent ? "bg-surface-alt" : ""}>
                      <button onClick={() => togglePage(page)} className="w-full flex items-center justify-between px-4 py-2 hover:bg-surface-alt transition-colors">
                        <div className="flex items-center gap-2">
                          <ChevronRight className={`w-3 h-3 text-text/40 transition-transform ${isExpanded ? "rotate-90" : ""}`} />
                          <span className={`text-xs font-medium ${isCurrent ? "text-primary" : "text-text/60"}`}>Page {page}</span>
                        </div>
                        <span className="text-xs text-text/30">{students.length}</span>
                      </button>
                      {isExpanded && (
                        <div className="pb-2">
                          {students.map((s) => (
                            <div key={s.id} className="flex items-center gap-2.5 px-4 py-1.5 pl-10">
                              {s.avatar_image_url ? (
                                ...
                                <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[primary] to-[primary] flex items-center justify-center text-xs font-bold text-text shrink-0">
                                  {s.full_name.charAt(0).toUpperCase()}
                                </div>
                              )}
                              <div className="min-w-0">
                                <div className="text-xs text-text/80 truncate">{s.full_name}</div>
                                {s.major && <div className="text-xs text-text/30 truncate">{s.major}</div>}
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          <div className="px-4 py-2 border-t border-border shrink-0">
            <div className="text-xs text-text/30 text-center">{allPages.length} pages with students</div>
          </div>
        </div>
      </div>
    );
  ...
  if (!collapsed) {
    return (
      <>
        ...
        {/* Desktop sidebar */}
        <div className="hidden md:flex fixed right-0 top-0 h-full z-30 w-72 flex-col bg-black/40  border-l border-border">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4 text-[primary]" />
              <span className="text-sm font-semibold text-text">Students</span>
  ...
  return (
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-30">
      ...
      >
        <ChevronRight className="w-4 h-4 text-text/60 rotate-180" />
      </button>
    </div>
  );
}
