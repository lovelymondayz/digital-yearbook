    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-text">Yearbooks</h1>
            <p className="text-text/50">Manage your university yearbooks</p>
          </div>
        ...
        {showCreate && (
          <motion.form
            ...
            className="card mb-8"
          >
            <h3 className="text-lg font-semibold text-text mb-4">Create New Yearbook</h3>
        ...
        <div className="space-y-4">
          {yearbooks.length === 0 ? (
            <div className="card text-center py-12">
              <BookOpen className="w-12 h-12 text-text/20 mx-auto mb-3" />
              <p className="text-text/50">No yearbooks yet. Create your first one!</p>
            </div>
          ) : (
            yearbooks.map((yb) => (
              <div key={yb.id} className="card flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-text">{yb.title}</h3>
                  <p className="text-text/50 text-sm">{yb.year} · {yb.status}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button className="p-2 rounded-lg hover:bg-surface-alt text-text/50 hover:text-text transition-colors">
                    <Eye className="w-4 h-4" />
                  </button>
                  <button className="p-2 rounded-lg hover:bg-surface-alt text-text/50 hover:text-text transition-colors">
                    <Edit className="w-4 h-4" />
                  </button>
                  <button className="p-2 rounded-lg hover:bg-red-500/10 text-text/50 hover:text-red-400 transition-colors">
